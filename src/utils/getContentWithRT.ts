...
export const getReadingTime = async (contentType: ContentType) => {
    const globContent = globPatterns[contentType] as Record<string, { default: CollectionEntry<typeof contentType> }>;
...
const getContentWithRT = async <T extends ContentType>(contentType: T, contents: CollectionEntry<T>[]) => {
